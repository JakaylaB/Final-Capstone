...
console.log("Another random integer between 10 and 200");
...
*/
function showUp() {
  let delay = 0; setDelay(normal);
  const hole = 0; chooseHoles(hole);
...
function showAndHide(hole, delay) {
  // Call the toggleVisibility function to show the mole
      const timeoutID = setTimeout(() => {
      toggleVisibility(hole); // Hide the mole
      gameOver(); // Check if my game should continue or stop
    }, delay);
...
function toggleVisibility(hole) {
  hole.classList.toggle('show'); // Show or hide the mole
  return hole; 
}
...
function showUp() {
  function showUp() {
    let delay = setDelay(); // Setting the delay
    const hole = chooseHole(); // Get the hole
    return showAndHide(hole, delay); // Show and hide the element
  }
}
...
*/
  ...
  function clearScore() {
    let points = 0; // Reset points to 0
...
*/
  ...
  function whack(event) {
   updateScore(); // Calling the update score when the mole is clicked
  console.log("whack")
   return points
  }
